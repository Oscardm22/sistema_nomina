...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $datos['empresa_id'] = intval($_POST['empresa_id'] ?? 0);
    $datos['nombre'] = limpiar($_POST['nombre'] ?? '');
    $datos['apellidos'] = limpiar($_POST['apellidos'] ?? '');
    $datos['fecha_nacimiento'] = limpiar($_POST['fecha_nacimiento'] ?? '');
    $datos['fecha_ingreso'] = limpiar($_POST['fecha_ingreso'] ?? date('Y-m-d'));
    $datos['puesto'] = limpiar($_POST['puesto'] ?? '');
    $datos['departamento'] = limpiar($_POST['departamento'] ?? '');
    $datos['salario_diario'] = floatval($_POST['salario_diario'] ?? 0);
    $datos['tipo_contrato'] = limpiar($_POST['tipo_contrato'] ?? 'Tiempo Indeterminado');
    $datos['regimen_contratacion'] = limpiar($_POST['regimen_contratacion'] ?? 'Sueldos');
    $datos['tipo_jornada'] = limpiar($_POST['tipo_jornada'] ?? 'Diurna');
    $datos['periodicidad_pago'] = limpiar($_POST['periodicidad_pago'] ?? 'Quincenal');
    $datos['banco'] = limpiar($_POST['banco'] ?? '');
    $datos['cuenta_bancaria'] = limpiar($_POST['cuenta_bancaria'] ?? '');
        ...
        try {
            $stmt = $conn->prepare("
                INSERT INTO empleados (
                    empresa_id, nombre, apellidos, fecha_nacimiento, fecha_ingreso,
            ...
            if ($resultado) {
                ...
                $mensaje = 'success|Empleado registrado exitosamente.';
                ...
                $errores[] = 'Error al guardar el empleado';
            }
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo BASE_URL; ?>modules/empleados/styles/formulario.css">
...
<body class="bg-gray-50">
            ...
            <div class="container mx-auto px-4 py-8">
                ...
                <div class="mb-8">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h1 class="text-3xl font-bold text-gray-900">Registrar Empleado</h1>
                            <p class="text-gray-600 mt-2">Completa la información del nuevo empleado</p>
                        </div>
                        <div>
                            <a href="index.php" 
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <?php endif; ?>
                                <p class="text-sm text-gray-500 mt-1">Salario base diario en Bolívares</p>
                            </div>
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <div class="form-section">
                            <h2 class="text-xl font-semibold">
                                ...
                                4. Información Bancaria (Opcional)
                            </h2>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <?php endif; ?>
                                <p class="text-sm text-gray-500 mt-1">Para depósito de nómina (opcional)</p>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Información sobre cálculo de salario -->
                <div class="mt-8 info-box">
                    <h4 class="text-lg font-semibold mb-2">
                        <i class="fas fa-calculator mr-2"></i>Información sobre el Salario
                    </h4>
                    <p class="text-sm text-gray-700 mb-3">
                        El salario diario es el monto base que se utilizará para calcular:
                    </p>
                    <ul class="list-disc ml-6 text-sm text-gray-700 space-y-1">
                        <li><strong>Salario semanal:</strong> Salario diario × 7 días</li>
                        <li><strong>Salario quincenal:</strong> Salario diario × 15 días</li>
                        <li><strong>Salario mensual:</strong> Salario diario × 30 días (promedio)</li>
                        <li><strong>Prestaciones sociales:</strong> Se calculan sobre este monto</li>
                        <li><strong>Vacaciones y utilidades:</strong> Proporcionales al salario diario</li>
                    </ul>
                    <p class="text-sm text-gray-700 mt-3">
                        <strong>Nota:</strong> Este monto debe incluir todas las asignaciones y comisiones fijas.
                    </p>
                </div>
                
                <!-- Información sobre tipos de contrato -->
                <div class="mt-4 info-box">
                    <h4 class="text-lg font-semibold mb-2">
                        <i class="fas fa-file-contract mr-2"></i>Tipos de Contrato en Venezuela
                    </h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                            <p class="font-medium text-gray-800">Tiempo Indeterminado</p>
                            <p class="text-gray-600">Contrato sin fecha de terminación específica.</p>
                        </div>
                        <div>
                            <p class="font-medium text-gray-800">Tiempo Determinado</p>
                            <p class="text-gray-600">Contrato con fecha de inicio y finalización específica.</p>
                        </div>
                        <div>
                            <p class="font-medium text-gray-800">Por Obra</p>
                            <p class="text-gray-600">Contrato para la realización de una obra o servicio específico.</p>
                        </div>
                        <div>
                            <p class="font-medium text-gray-800">Honorarios</p>
                            <p class="text-gray-600">Contrato para servicios profesionales por honorarios.</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        // Calcular edad automáticamente si se completa fecha de nacimiento
        document.getElementById('fecha_nacimiento').addEventListener('change', function() {
            if (this.value) {
                const fechaNac = new Date(this.value);
                const hoy = new Date();
                ...
                // Mostrar edad calculada (opcional)
